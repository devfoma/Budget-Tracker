...
Budget Tracker: Design and Implementation of a Mobile-Based Budget and Expenses Tracking System
...
Budget Tracker is a mobile-based budget and expense tracking application that helps users plan budgets, record income and expenses, monitor spending habits, and view financial reports through simple visual dashboards.
...
The application will not use server authentication. Each local account/profile created on a device will be assigned a unique local user ID, and all budgets, transactions, categories, notifications, and settings will be stored against that ID. This allows more than one person to use Budget Tracker on the same phone while keeping their records separated in local storage.
...
- Multiple local profiles/accounts on the same device, separated by unique local user IDs.
...
- Server authentication, online login, cloud accounts, or remote account sync.
- Server-hosted user data.
- Cloud backup unless added as a future optional export/import feature.
...
- Assign every local profile a unique generated user ID.
...
- Store and load each profile's records using its unique local user ID.
...
| user_id | text | Owner local user ID for custom category; null for default local categories |
...
|---|---|---|
| id | text | Primary key generated by the app |
| user_id | text | Local user ID that owns the budget |
...
|---|---|---|
| id | text | Primary key generated by the app |
| user_id | text | Local user ID that owns the transaction |
...
|---|---|---|
| id | text | Primary key generated by the app |
| user_id | text | Local user ID that owns the notification |
...
| Field | Type | Description |
|---|---|---|
| user_id | text | Primary key and local user ID that owns the settings |
...
- One local user ID has one profile.
- One local user ID has many transactions.
- One local user ID has many budgets.
- One local user ID has many custom categories.
- One category has many transactions.
- One budget may be linked to one category.
- One budget may have many transactions.
- One local user ID has many notifications.
- One local user ID has one settings record.
...
2. App checks whether any local profiles exist.
3. If no profile exists, user is sent to onboarding and Create Profile.
4. User creates a local profile, receives a unique local user ID, and selects preferred currency.
...
7. App stores the active local user ID in local preferences.
8. App loads profile, settings, categories, budgets, and dashboard summary for the active local user ID.
9. User enters the main app.
...
4. Every user-owned record includes the active local user ID so accounts on the same phone remain separated.
5. Optional future backup/export can generate CSV or JSON files.
6. Optional future backup/import would require user confirmation for sensitive changes.

## 17. Local Security Plan

### Local Access Control

- Do not implement server authentication, online login, or cloud account sessions.
- Use local profiles/accounts protected by optional PIN or biometric app lock.
- Generate a unique local user ID for every profile/account created on the device.
...
| Table | Security Rule |
|---|---|
| profiles | Store each local profile with a unique generated local user ID |
| budgets | Store records under the owning local user ID |
| transactions | Store records under the owning local user ID |
| notifications | Store records under the owning local user ID |
| user_settings | Store settings under the owning local user ID |
| categories | Store default categories globally and custom categories under the owning local user ID |
...
- A transaction must belong to the active local user ID.
- User-owned records must always be queried by active local user ID.
...
| FR-02 | The system shall generate a unique local user ID for each local profile/account. |
...
| FR-14 | The system shall allow users to lock and unlock the app locally when app lock is enabled. |
...
- Email must be valid.
- PIN must meet the required local app-lock length when app lock is enabled.
...
|---|---|
| User creates a local profile successfully | User enters the dashboard |
...
- Create profile and settings records using generated local user IDs.
...
- A user can create a local profile/account and lock or unlock the app locally.
- Each local profile/account receives a unique local user ID, and records remain separated by that ID.
...
- User data is protected so one local profile/account cannot access another local profile/account's records.
